...
def gen_attached_report_name(json_path) -> str:
    """
...
class JSONExporter(Exporter):
    ...
    def export(
        ...
        if not source.is_empty():
            ...
            if self.cfg.split_json_report:
                structure_filename = gen_attached_report_name(json_path)
                ...
                for multitest_key, assertions in assertions_map.items():
                    meta["attachments"][multitest_key] = str(
                        attachments_dir / multitest_key
                    )
                    with (attachments_dir / multitest_key).open(
                        "w"
                    ) as json_file:
    ...
    def split_json_report(data):
        ...
        meta["entries"] = []
        for multitest in structure:
            multitest_key = "assertions_{}".format(multitest["uid"])
            assertions_map[multitest_key] = {}
            JSONExporter.split_assertions(
                multitest["entries"], assertions_map[multitest_key]
            )
    ...
    def merge_json_report(meta, structure, assertions_map, strict=True):
        ...
        for multitest in structure:
            multitest_key = "assertions_{}".format(multitest["uid"])
            assertions = assertions_map.get(multitest_key, {})
            JSONExporter.merge_assertions(
                multitest["entries"], assertions, strict
            )
